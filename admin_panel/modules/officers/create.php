...
$name = $designation = $description = $order_position = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($name_err) && empty($designation_err) && empty($description_err) && empty($order_err) && empty($media_err)){
        $sql = "INSERT INTO officers (name, designation, description, image_path, order_position, is_active) VALUES (?, ?, ?, ?, ?, 1)";
        
        if($stmt = mysqli_prepare($conn, $sql)){
            mysqli_stmt_bind_param($stmt, "ssssi", $param_name, $param_designation, $param_description, $param_image_path, $param_order_position);
