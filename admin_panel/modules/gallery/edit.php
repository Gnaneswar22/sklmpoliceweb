...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(isset($_FILES["media"]) && $_FILES["media"]["error"] != 4){
        ...
        if(empty($media_err) && in_array($filetype, $allowed)){
        ...
        $media_path = $current_image;
    }
    
    // Update record
    if(empty($title_err) && empty($media_err)){
        $sql = "UPDATE gallery SET title=?, media_path=?, alt_text=? WHERE id=?";
        
        if($stmt = mysqli_prepare($conn, $sql)){
            mysqli_stmt_bind_param($stmt, "sssi", $param_title, $param_media_path, $param_alt_text, $param_id);
            ...
            if(mysqli_stmt_execute($stmt)){
                $success_msg = "Gallery item updated successfully.";
            } else{
...
<head>
    ...
    <style>
        /* Include the same styles as create.php */
