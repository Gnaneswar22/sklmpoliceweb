...
$title = $description = $link = $order_position = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($title_err) && empty($description_err) && empty($order_err) && empty($media_err)){
        $sql = "INSERT INTO services (title, description, icon_path, link, order_position, is_active) VALUES (?, ?, ?, ?, ?, 1)";
        
        if($stmt = mysqli_prepare($conn, $sql)){
            mysqli_stmt_bind_param($stmt, "ssssi", $param_title, $param_description, $param_icon_path, $param_link, $param_order_position);
