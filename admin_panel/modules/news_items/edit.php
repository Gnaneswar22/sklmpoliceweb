...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($title_err) && empty($content_err) && empty($date_err)){
        $sql = "UPDATE news_items SET title=?, content=?, news_date=? WHERE id=?";
        
        if($stmt = mysqli_prepare($conn, $sql)){
            mysqli_stmt_bind_param($stmt, "sssi", $param_title, $param_content, $param_news_date, $param_id);
            ...
            if(mysqli_stmt_execute($stmt)){
                $success_msg = "News item updated successfully.";
            } else{
